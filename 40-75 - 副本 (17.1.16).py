...
def height(zeta,Phi):
	theta_mu,psi=viewToMagnetic(zeta,Phi)
	sintheta=sin(theta_mu)
	if cos(theta_mu)>=0:
		cosine=(-sintheta**2+sqrt(sintheta**4-10*sintheta**2+9))/3
	else:
...
def area(Phi1,Phi2):
	...
	for Phi in linspace(Phi1,Phi2,100):
		theta_mu,psi=viewToMagnetic(zeta_view,Phi)
		sintheta=sin(theta_mu)
		if cos(theta_mu)>=0:
			cosine=(-sintheta**2+sqrt(sintheta**4-10*sintheta**2+9))/3
		else:
		...
		if theta>theta_A:
			...
			theta2=theta
		Theta,none=magneticToView(theta,psi)
...
for Phi in Philist:
	...
	none,psi=viewToMagnetic(zeta_view,Phi)
